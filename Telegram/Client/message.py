...
def broadcast_messages(coin_id: int, analysis_id: int, timeframe_id: int, position: str, target_price: float,
                       current_price: float, risk: str):
    users = functions.get_user_recommendation(coin_id=coin_id, analysis_id=analysis_id, timeframe_id=timeframe_id)
    coin = functions.get_coin_name(coin_id)
    analysis = functions.get_analysis(analysis_id)
    timeframe = functions.get_timeframe(timeframe_id)
    ...
    for user in users:
        ...
        try:
            bot.send_message(chat_id=int(functions.get_user_chat_id(user[0])), text=message,
                             parse_mode='Markdown')

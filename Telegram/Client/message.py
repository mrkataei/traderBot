...
"""
from Inc.functions import get_user_strategy, get_coins, get_analysis, get_timeframes, get_chat_ids
...
def broadcast_messages(coin_id: int, analysis_id: int, timeframe_id: int, position: str
    ...
    coin = get_coins(coin_id)
    analysis = get_analysis(analysis_id)
    timeframe = get_timeframes(timeframe_id)
    for user in users:
        message = f'👋🏼 {trans("C_hello")} {user[0]}!\n💥{trans("M_new_signal")}*{analysis[0][1]}*!!!\n' \
                  f'*{coin[0][0]}* {trans("C_now")} {trans("M_in")} *{position}* {trans("M_position")}\n' \
                  f'{trans("M_current_price")}: {current_price}$\n' \
                  f'{trans("M_risk")}: *{risk}*\n' \
                  f'{trans("C_timeframe")}: {timeframe[0][0]}'
        try:
            bot_ins.send_message(chat_id=int(get_chat_ids(user[0])), text=message,
                                 parse_mode='Markdown')

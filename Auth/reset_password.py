...
def reset_password(db_connection: MySQLConnection, username: str, answer: str, new_password: str, new_password2: str):
    ...
    def check_answer():
        try:
            query = 'SELECT * FROM users WHERE username="{username}" AND question_answer="{answer}" LIMIT 1'.format(
    ...
    def set_password():
        ...
        if not chek_pass[0]:
            ...
            try:
                ...
                sql = 'UPDATE users SET password="{password}" , salt="{salt}" WHERE username="{username}"'.format(
                    password=password[0], salt=password[1], username=username)
                cur.execute(sql)

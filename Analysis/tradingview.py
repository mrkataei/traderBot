...
class TradingView:
    ...
    def __init__(self , symbol  ,time_frame , screener , exchange):
        ...
        self.comp=TA_Handler(symbol=self.symbol ,screener=self.screener ,exchange=self.exchange , interval=self.interval)
    ...
    def get_moving_averages_all_rec(self , recommendation='all'):
        indi = self.comp.get_analysis().moving_averages
        all = {'RECOMMENDATION' : indi['RECOMMENDATION'] ,'BUY': indi['BUY']  , 'SELL': indi['SELL'] , 'NEUTRAL':indi['NEUTRAL']}
        return {
